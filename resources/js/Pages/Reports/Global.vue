<template>
    <AuthenticatedLayout>
        <Head title="Global Time Reports" />
        ...
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Organisation-wide time tracking overview</p>
                </div>
                <button
                    type="button"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                <form @submit.prevent="applyFilters" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div>
                        ...
                        <select v-model="filterForm.project_id" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm">
                    ...
                    <button type="button" @click="resetFilters" class="rounded-md bg-white dark:bg-gray-700 px-3 py-2 text-sm font-semibold text-gray-700 dark:text-gray-300 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50">
                        Reset
                    </button>
                </form>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
                    ...
                    <div class="mt-2 text-3xl font-bold text-indigo-600 dark:text-indigo-400">{{ formatMinutes(totalMinutes) }}</div>
                    <div class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ filteredLogs.length }} log entries</div>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
                    <div class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Projects Tracked</div>
                    <div class="mt-2 text-3xl font-bold text-green-600 dark:text-green-400">{{ byProject.length }}</div>
                    <div class="mt-1 text-xs text-gray-400 dark:text-gray-500">across the period</div>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                    <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                        <h2 class="text-sm font-semibold text-gray-900 dark:text-white">Time by Project</h2>
                    </div>
                    <div class="p-5 space-y-3">
                        <div v-if="byProject.length === 0" class="text-sm text-gray-400 dark:text-gray-500 italic text-center py-4">No data for selected period.</div>
                        <div v-for="item in byProject" :key="item.project_id" class="flex items-center gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between mb-1">
                                    <span class="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{{ item.project_name }}</span>
                                ...
                                <div class="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                                    <div
                                        ...
                                        :style="{ width: maxProjectMinutes ? (item.total_minutes / maxProjectMinutes * 100) + '%' : '0%' }"
                                    ></div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                    <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                        <h2 class="text-sm font-semibold text-gray-900 dark:text-white">Time by User</h2>
                    </div>
                    <div class="p-5 space-y-3">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-700/50">
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Project</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            <tr v-if="filteredLogs.length === 0">
                                <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-400 dark:text-gray-500 italic">No time logs found.</td>
                            </tr>
                            <tr v-for="log in filteredLogs" :key="log.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">{{ projectName(log.project_id) }}</td>
                        ...
                        <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <select v-model="logForm.task_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm">
                                    ...
                                    <optgroup v-for="p in tasksGrouped" :key="p.project_id" :label="p.project_name">
                                        <option v-for="t in p.tasks" :key="t.id" :value="t.id">{{ t.title }}</option>
                                    </optgroup>
                                </select>
                            ...
                            <div class="flex justify-end gap-3 pt-2">
                                ...
                                <button type="button" :disabled="logProcessing" @click="submitGlobalLog" class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 disabled:opacity-50">
...
const filteredLogs = computed(() => {
    let list = [...props.logs];
    if (filterForm.value.project_id) {
        list = list.filter(l => l.project_id == filterForm.value.project_id);
    }
...
const totalMinutes = computed(() => filteredLogs.value.reduce((s, l) => s + l.time_spent, 0));

const maxProjectMinutes = computed(() => Math.max(...props.byProject.map(p => p.total_minutes), 1));
const maxUserMinutes    = computed(() => Math.max(...props.byUser.map(u => u.total_minutes), 1));

const applyFilters = () => {
    router.get(route('reports.global'), {
...
// --- Tasks grouped by project for the modal optgroup ---
const tasksGrouped = computed(() => {
    const map = {};
    for (const t of props.tasks) {
        const p = t.project;
        if (!p) continue;
        if (!map[p.id]) map[p.id] = { project_id: p.id, project_name: p.name, tasks: [] };
        map[p.id].tasks.push(t);
    }
...
const submitGlobalLog = () => {
    ...
    // Find task's project
    const task = props.tasks.find(t => t.id == logForm.value.task_id);
    if (!task) { logErrors.value.task_id = 'Task not found.'; return; }
    const project = props.projects.find(p => p.id === task.project_id);
    if (!project) { logErrors.value.task_id = 'Project not found.'; return; }

    logProcessing.value = true;
    router.post(
        route('project.tasks.time-logs.store', { projectId: project.project_id, task: task.id }),
